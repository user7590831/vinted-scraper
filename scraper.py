...
parser.add_argument(
    "--depop", "-d", dest="Depop", action="store_true", help="Download Depop data."
)
parser.add_argument(
    "--private_msg",
    "-p",
    dest="priv_msg",
    action="store_true",
    help="Download images from private messages from Vinted",
)
parser.add_argument(
    "--user_id",
    "-u",
...
parser.add_argument(
    ...
    dest="session_id",
    action="store",
    help="Session id cookie for Vinted",
    required=False,
)
parser.add_argument(
    "--disable-file-download",
    "-n",
    dest="disable_file_download",
    action="store_true",
    help="Disable file download (Currently only working for depop)",
    required=False,
)
parser.add_argument(
    "--sold_items",
    ...
    dest="sold_items",
    action="store_true",
    help="Also download sold items (depop)",
    required=False,
)
parser.add_argument(
    "--start_from",
    ...
    dest="start_from",
    action="store",
    help="Begin from a specific item (depop)",
    required=False,
)
parser.add_argument(
    "--maximum_images",
    ...
    dest="maximum_images",
    action="store",
    help="Set a maximum amount of image to download. 1 image by default (vinted)",
    required=False,
)
parser.add_argument(
    "--debug",
...
if args.disable_file_download and not args.Depop:
    logging.error(
        "-n only works with Depop. Use -n -d to disable filedownloads from Depop"
    )
    exit(1)
...
c.execute(
    """CREATE TABLE IF NOT EXISTS Depop_Users
...
def update_col():
    logging.info(
        "Updating columns of Data Table with new version of the script: add Url and Favourite field"
    )
...
def download_vinted_data(userids, s):
    ...
    for user_id in userids:
        ...
        if r.status_code == 200:
            ...
            verification_email = (
                data.get("verification", {}).get("email", {}).get("valid", None)
            )
            verification_facebook = (
                data.get("verification", {}).get("facebook", {}).get("valid", None)
            )
            verification_google = (
                data.get("verification", {}).get("google", {}).get("valid", None)
            )
            verification_phone = (
                data.get("verification", {}).get("phone", {}).get("valid", None)
            )
                ...
                if products:
                    ...
                    try:
                        ...
                        if os.path.exists(path):
                            ...
                            logging.error(
                                f"Creation of the directory {path} failed: {e}"
                            )
                    else:
                        ...
                        if images:
                            ...
                            if args.maximum_images:
                                try:
                                    ...
                                    logging.error(
                                        "Invalid value for maximum_images: This argument needs to be a number"
                                    )
                                    continue
                                if count_img <= 0:
                                    logging.error(
                                        "Maximum images must be greater than 0"
                                    )
                                    continue
...
def get_all_depop_items(
    data, baseurl, ids_list, args, begin, s, data_key="objects", id_key="id"
):
    if args.start_from:
...
def download_depop_data(userids):
    ...
    for userid in userids:
        userid = userid.strip()
        search_data = s.get(
            f"https://api.depop.com/api/v1/search/users/top/?q={userid}"
        ).json()
        item = None
        ...
        baseurl = (
            f"https://api.depop.com/api/v1/users/{real_userid}/products/?limit=200"
        )
        ...
        begin = False
        product_ids = []
        product_ids = get_all_depop_items(
            data, baseurl, product_ids, args, begin, s, data_key="objects", id_key="id"
        )
        ...
        for product_id_ in product_ids:
            ...
            try:
                ...
                if product_data.status_code == 200:
                    ...
                    logging.warning(
                        f"Unexpected status code: {product_data.status_code}"
                    )
                    continue
            except ValueError:
            ...
            for images in product_data["pictures_data"]:
                ...
                if not args.disable_file_download:
                    if not os.path.isfile(filepath):
                        ...
                        if result:
                            c.execute(
                                """UPDATE Depop_Data SET Image = ? WHERE ID = ?""",
                                (filepath, product_id),
                            )
                            conn.commit()
                            req = requests.get(full_size_url)
